...
const newFormArray = config => {
  if (config[IS_MULTIPLE]) checkKey(false, "(newFormArray) Invalid form schema", "object");
  return {
    [IS_MULTIPLE]: true,
...
var useConsumerHook = (function () {
  ...
  React.useEffect(() => {
    if (typeof formRef._formConfig[props[idKey || ID_KEY]].validate === false && props.dontValidateOnUnmount) formRef.setValidate({
  ...
  }, [formRef._formId_, props.resetOnUnmount, props.dontValidateOnUnmount, props[idKey || ID_KEY]]);
...
const generateNewFormRef = (formRef, _values, __formRef, rootFormRef, isMultiple) => {
  const __values = typeOf(_values) === TYPE_OBJECT$1 || _values === undefined || _values === null ? _values : {
...
var MultipleFormProvider = /*#__PURE__*/React.forwardRef((_ref2, ref) => {
  ...
  const generate = () => {
    const values = formRef.getValues && formRef.getValues().length ? formRef.getValues() : formRef[IS_MULTIPLE]._initialValues;
    let _formArrayRef = [];
    if (values.length > 0) {
      _formArrayRef = values.map((_values, _index) => {
        if (formRef[IS_MULTIPLE]?._formRefArray?.[_index]?.formRef) {
          const _existingFormRef = formRef[IS_MULTIPLE]?._formRefArray?.[_index];
          return _existingFormRef;
        }
        return generateNewFormRef(formRef, _values, ___formRef, rootFormRef, !!___formRef._multipleConfig);
      });
    } else {
      _formArrayRef = [...Array(typeof defaultCount === "number" ? defaultCount : 0)].map(_values => generateNewFormRef(formRef, _values, ___formRef, rootFormRef, !!___formRef._multipleConfig));
    }
    if (formRef[IS_MULTIPLE]) formRef[IS_MULTIPLE]._formRefArray = _formArrayRef;
  ...
  const onAddMultipleForm = function () {
    ...
    if (startIndex >= 0 && startIndex <= valueRef.current.formRefArray.length) setFormRefArray(_val => {
      ...
      if (typeof startIndex === "number") __val.splice(startIndex, 0, ..._newFormRef);else {
  ...
  const onDeleteMultipleForm = function () {
    ...
    if (deleteKey.length > 0) setFormRefArray(_options => {
      ...
      __options = __options.filter(_ref4 => {
        let {
          formId
        } = _ref4;
        return !deleteKey.includes(formId);
      });
      if (formRef[IS_MULTIPLE]) formRef[IS_MULTIPLE]._formRefArray = __options;
      return __options;
    });
  };
  ...
  const setInitialFormData = React.useCallback(function () {
    ...
    if (value.length > valueRef.current.formRefArray.length) {
      ...
      if (formRef[IS_MULTIPLE]) formRef[IS_MULTIPLE]._formRefArray = _formRefArray;
  ...
  const ChildrenComponentWithProps = (_ref, _i) => {
    ...
    return typeof children === "function" ? children(..._props) : /*#__PURE__*/React__default.cloneElement(children, {
      formProps: _props
    }, null);
  };
  return /*#__PURE__*/React__default.createElement(FormContext.Provider, {
  ...
  }, noAutoLoop ? typeof children === "function" ? children(formRefArray, _multipleProps) : children : formRefArray.map(ChildrenComponentWithProps)));
});
...
const formValidationHandler = function () {
  ...
  const _formValidationHandler = function () {
    ...
    const validateValue = function (__value, key, isSetValue, isSetError, _config) {
      ...
      let error = null;
      if ((typeof config.validate === "boolean" ? config.validate : true) && (config._initiated || ALLOW_WITHOUT_INITIATED) && (typeof config.isValidateOnChange === "boolean" ? config.isValidateOnChange : true)) {
        error = validatorError;
    ...
    const validateForm = props => {
      ...
      obj = Object.entries(formRef.current._schema).reduce((acc, _ref4) => {
        ...
        const _formRef = val.formRef[IS_MULTIPLE].validateForm ? val.formRef[IS_MULTIPLE] : val.formRef;
    ...
    const commonInputProps = function (key) {
      ...
      _commonInputProps = {
        ...
        ...(INITIAL_FORM_CONFIG && (typeof INITIAL_FORM_CONFIG.inputProps === "function" ? INITIAL_FORM_CONFIG.inputProps(formRef.current, INITIAL_FORM_CONFIG._config, {
          ...
          key
        }, formRef.current._extraProps || __formRef.current._extraProps) : INITIAL_FORM_CONFIG.inputProps) || {})
      };
    ...
    const getValues = () => {
      return getOriginalFormObject(Object.entries(formRef.current.getFormConfig()).reduce((acc, _ref13) => {
        ...
        if ((!_isSchema && !(val._initiated || ALLOW_WITHOUT_INITIATED) || _isSchema && !(formRef.current._schema[key] && (formRef.current._schema[key].formRef._is_form_initiated || ALLOW_WITHOUT_INITIATED))) && !_isSchema_or_isMultiple_config_is_root) return acc;
        if (!_isSchema && !(val._initiated || ALLOW_WITHOUT_INITIATED)) return acc;
  ...
  const useFormValidationHook = _ref18 => {
    ...
    formRef._ref(IS_FORMREF).setInitialState = (_initialState, _initialErrors) => {
      setState(() => initiateFormValidationHandler(_initialState, _initialErrors));
    };
    formRef._ref(IS_FORMREF).save = function () {
      let is_init = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
      valueRef.current = {
        ...valueRef.current,
        ...formRef.getValues()
      };
