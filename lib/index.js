...
const _setInitialValues = _ref => {
  ...
  const _values = (isMultiple ? Array.isArray(_initialValues) ? _initialValues : [_initialValues] : [_initialValues]).reduce((acc, initialValues) => {
    ...
    const __values = Object.entries(formConfig || {}).reduce((acc, _ref2) => {
      let [key, val = {}] = _ref2;
      if (typeOf(key) === TYPE_OBJECT$1 && key in initialValues) return newObject(acc, {
...
function _deepCopy(src, /* INTERNAL */_visited, _copiesVisited) {
...
var FormConsumer = (_ref3 => {
  ...
  return props.id && !props.render ? /*#__PURE__*/React__default.createElement(Consumer, useConsumerHook(props), children) : /*#__PURE__*/React__default.createElement(ConsumerWithoutId, useConsumerHook(props), children);
});
...
var FormProvider = /*#__PURE__*/React.forwardRef((_ref2, ref) => {
  ...
  // (_formRef && _formRef.constructor.name !== "FormRef")
    ...
    if (_formRef) throw new Error("Invalid FormRef");else throw new Error("Required props 'formRef' or 'id' ");
  }
  const {
    formRef: __formRef,
...
const formValidationHandler = function () {
  ...
  const _formValidationHandler = function () {
    ...
    const _resetValue = isClear => key => {
      const config = formConfig[key] || {};
      return isClear ? "default" in config ? config.default : "" : typeOf(key) === TYPE_OBJECT$1 && key in initialValues ? typeof initialValues[key] === "function" ? initialValues[key]() : initialValues[key] : "default" in config ? config.default : "";
    };
    ...
    const _onFormChangeCallback = dontRender => {
      ...
      if (typeOf(onFormChangeCallback) === TYPE_FUNCTION$1 && !dontRender) setTimeout(() => {
        if (__formRef.current.getValues) {
          const _data = {
    ...
    const resetFormInput = function () {
      ...
      clearKeys.forEach(_key => {
        if (isDeleteKey && typeOf(_key) === TYPE_OBJECT$1 && !(_key in initialValues)) {
          delete _values[_key];
          delete _errors[_key];
        } else {
    ...
    const setInitialFormData = (value, isResetValue) => {
      ...
      const _values = Object.entries(formRef.current.formConfig).reduce((acc, _ref6) => {
        ...
        if (val[IS_SCHEMA] || val._formId_ || val[IS_MULTIPLE]) {
          formRef.current._schema[key].formRef.setInitialFormData(_value[key], isResetValue);
          return acc;
        }
        return newObject(acc, {
          [key]: key in _value ? _value[key] : isResetValue ? formRef.current._resetValue(key) : _value[key]
        });
      }, {});
      setValues(_values);
    };
